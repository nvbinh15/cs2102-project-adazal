...
SELECT c.id as id, c.user_id as user_id, r.other_comment_id as other_comment_id, rv.reply_timestamp, rv.content
from reply r, comment c, reply_version rv
where c.user_id = 1 and r.id = c.id and rv.reply_id = r.id and r.other_comment_id = 6;

SELECT c.id as id, c.user_id as user_id, r.other_comment_id as other_comment_id, rv.reply_timestamp, rv.content
from reply r, comment c, reply_version rv
where c.user_id = 2 and r.id = c.id and rv.reply_id = r.id and r.other_comment_id = 11;
